...
exports.createClient = function (webSocketUri, clientOrigin) {
  var parsedWsUri = url.parse(webSocketUri);
  ...
  var socket = tcp.createConnection(parsedWsUri.port, parsedWsUri.hostname);
  socket.setTimeout(0);
  ...
  socket.addListener('connect', function() {
      ...
      path:     parsedWsUri.pathname,
      host:     parsedWsUri.hostname + ":" + parsedWsUri.port,
      origin:   clientOrigin,
    });
    socket.write(hs);
  });
  ...
  function handshake(data) {
	...
	// skip some spurious data that come every now and then
    var c = buffer.indexOf('\u0000');
    if(c >= 0) {
